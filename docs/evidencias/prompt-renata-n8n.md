# Prompt da Renata — extraído do n8n

Fonte: workflow `#1 Agente SDR | 10/02/26 | V2.2` (id `i5CHQ5VgzrA65kuK`), nó `AI Agent`.
...
| Temperatura | `0.3` |
| Memória | Postgres Chat Memory, sessão = telefone, janela de 12 mensagens |
| Saída | JSON estruturado, `messages` como array de strings, `minItems: 1` |

Schema do output parser:
...
## Dados do Lead Atual:
- Nome: {{ $('Fields').item.json.pushName }}
- Origem: {{ $('Add New Lead').item.json.source }}
- Telefone: {{ $('Fields').item.json.phone }}
- Data Hoje(dd/MM/yyyy): {{ $now.setZone('America/Sao_Paulo').toFormat('dd/MM/yyyy HH:mm:ss') }} ({{ $now.setZone('America/Sao_Paulo').toFormat('EEEE') }})
