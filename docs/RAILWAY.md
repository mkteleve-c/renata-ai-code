...
> Railway é o **caminho alternativo**. O caminho oficial é Docker + Traefik +
> Let's Encrypt em VPS própria — ver [`deploy/README.md`](../deploy/README.md)
> e [DEPLOY.md](DEPLOY.md). Use Railway quando quiser provisionamento gerenciado
> e iteração rápida sem tocar em servidor.
...
- Executa os agentes LangGraph e envia respostas pelo cliente outbound do canal de origem (lê `message_queue.channel`: twilio | meta | uazapi)
...
└── 006_message_channel.sql          # message_queue.channel (twilio | meta | uazapi)
```

Para adicionar uma nova migração, crie um arquivo SQL com o próximo número sequencial (ex: `007_nova_feature.sql`). A ordem alfabética dos nomes determina a ordem de aplicação.
...
| `OUTBOUND_MODE` | `real` | Modo outbound compartilhado entre Twilio/Meta/uazapi. `TWILIO_OUTBOUND_MODE` ainda funciona como alias legacy. |
...
> validação acontece): `TWILIO_AUTH_TOKEN`/`TWILIO_WEBHOOK_URL` e
> `META_APP_SECRET`/`META_VERIFY_TOKEN`. As credenciais outbound ficam **no
> serviço `worker`**: `TWILIO_*` (envio), `META_PHONE_NUMBER_ID`/`META_ACCESS_TOKEN`,
> `UAZAPI_BASE_URL`/`UAZAPI_INSTANCE_TOKEN`.

> **Fail-fast**: em `OUTBOUND_MODE=real`, API e Worker derrubam o boot se um
> canal está "tocado parcialmente" (ex: `META_VERIFY_TOKEN` preenchido mas
> `META_PHONE_NUMBER_ID` vazio). Para desabilitar um canal, **zere todas** as
> credenciais dele.
...
7. Configurar `OUTBOUND_MODE=real` no Worker; preencher credenciais de **pelo menos um** canal (Twilio, Meta ou uazapi); deixar canais não usados completamente vazios
8. Gerar domínio público para API e Frontend
9. Atualizar `TWILIO_WEBHOOK_URL` (e/ou Meta endpoint) com o domínio real da API
10. Atualizar `BETTER_AUTH_URL` com o domínio real do Frontend
11. Configurar watch paths por serviço (ver tabela acima)
12. Configurar 2 réplicas na API
13. Adicionar volume ao serviço DB (`/var/lib/postgresql/data`)
14. Verificar migrações (rodam automaticamente no startup da API — checar logs por `migration_applying`; sequência atual vai de `001_initial.sql` a `006_message_channel.sql`)
15. Testar health check: `GET https://api-*.up.railway.app/health`
16. Cadastrar webhook no(s) provedor(es): Twilio Console / Meta App / painel uazapi
17. Definir `ADMIN_EMAIL` e `ADMIN_PASSWORD` no serviço `frontend`
18. Acessar `/login`, validar o bootstrap automático do primeiro admin e trocar a senha
19. Testar fluxo completo: mensagem WhatsApp → fila → worker → resposta no canal de origem
