...
> Os exemplos usam o webhook **Twilio** porque ele é o mais didático
> (form-encoded, fácil de simular pelo Swagger). Os mesmos passos valem para
> `/webhook/meta` e `/webhook/uazapi` — só muda a forma do payload e a
> assinatura. Em produção, prefira o **pgweb em `/banco`** ao invés de `psql`
> direto (ver [DATABASE.md](DATABASE.md)).
...
3. No campo `agent` (query param), digite o ID do agente que quer testar — o
   catálogo atual tem `illumi_assistant` e `rhawk_assistant`. Os exemplos
   abaixo usam `rhawk_assistant`, mas substitua pelo seu agente.
...
**Em dev local** (Docker compose padrão), conecte com `psql`:

```bash
docker compose exec db psql -U postgres -d whatsapp_langchain
```

**Em produção** (Docker + Traefik), use o **pgweb em `/banco`** — interface
web com aba SQL, protegida por BasicAuth (`PGWEB_AUTH_USER`/`PGWEB_AUTH_PASS`
do `.env.prod`). É o caminho oficial; Postgres não é exposto publicamente.

### 3.1 — Ver a mensagem na fila

```sql
SELECT id, phone_number, agent_id, channel, status,
       incoming_message, response, error
FROM message_queue
WHERE message_id = 'SM_TESTE_001';
```

`channel` mostra de qual webhook a mensagem veio (`twilio`/`meta`/`uazapi`)
e define qual cliente outbound o worker usou para responder.
