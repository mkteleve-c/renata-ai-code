...
O `locustfile.py` define duas classes de usuários virtuais, ambas batendo no
**webhook Twilio** (`/webhook/twilio`) para reaproveitar a infra de assinatura
HMAC-SHA1 já pronta. Os mesmos números (RPS, latência, drenagem da fila)
representam o harness completo — o canal de entrada é detalhe de protocolo:
quem dá trabalho é o agente + LLM + worker, e isso é canal-agnóstico.
...
|---------|----------|-------------|--------------|---------|
| **worker** | `OUTBOUND_MODE` | `mock` | `real` | Impede que o Worker envie mensagens reais pelos canais habilitados durante o teste. Sem isso, cada mensagem processada gera uma chamada real ao provedor (custo + spam). `OUTBOUND_MODE` afeta os três canais (twilio/meta/uazapi). |
...
> **Importante:** Após o teste, **reverta todas as variáveis** para os valores normais. Em especial, `OUTBOUND_MODE` deve voltar para `real` para que o bot funcione normalmente.
...
|---------|----------|--------------|
| **worker** | `OUTBOUND_MODE` | `real` |
| **worker** | `LLM_RATE_LIMIT_REQUESTS_PER_SECOND` | `0.5` |
| **worker** | `LLM_RATE_LIMIT_MAX_BURST` | `10` |
| **api** | `RATE_LIMIT_PER_HOUR` | `30` |

> **Cuidado:** Se esquecer de reverter `OUTBOUND_MODE` para `real`, o bot para de responder em **todos** os canais habilitados (mensagens são processadas mas as respostas são descartadas pelo cliente mock).
...
| **Worker** | 1 réplica, `OUTBOUND_MODE=mock`, `LLM_RATE_LIMIT_REQUESTS_PER_SECOND=5`, `LLM_RATE_LIMIT_MAX_BURST=20` |
