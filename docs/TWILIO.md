...
> Este projeto suporta também os canais **Meta WhatsApp Cloud API** ([META.md](META.md))
> e **uazapi/uazapiGO** ([UAZAPI.md](UAZAPI.md)). Os três coexistem e o roteamento
> outbound é automático por mensagem (`message_queue.channel`). Use Twilio se
> quer trabalhar via sandbox rápido ou se já tem número aprovado pela Twilio;
> Meta para usar a Cloud API direto; uazapi se não pode esperar aprovação Meta.
...
cloudflared tunnel ──► API (localhost:8000)               [Parte A — sandbox/local]
       ou
${DOMAIN} via Traefik ──► API (Docker + TLS Let's Encrypt) [Parte B — produção oficial]
       ou
Domínio Railway ──► API (api-*.up.railway.app)             [Parte B — Railway alternativo]
                              │
                              ▼
                       PostgreSQL (fila)
                              │
...
# OUTBOUND_MODE=mock em dev local / real para envio de verdade.
# Compartilhado entre Twilio/Meta/uazapi. TWILIO_OUTBOUND_MODE ainda
# funciona como alias retrocompatível, mas é melhor usar OUTBOUND_MODE.
OUTBOUND_MODE=mock
...
Em desenvolvimento local, `OUTBOUND_MODE=mock` permite validar fila,
worker, admin panel e E2E sem consumir cota do sandbox. Para testes reais de
WhatsApp, mude para `OUTBOUND_MODE=real` (afeta todos os canais habilitados).
...
# Envio real obrigatório (afeta todos os canais habilitados)
OUTBOUND_MODE=real

# Validação de assinatura obrigatória em produção
VALIDATE_TWILIO_SIGNATURE=true

# URL pública da API — apenas a base, sem path nem barra final.
# Deploy oficial Docker + Traefik:
TWILIO_WEBHOOK_URL=https://${DOMAIN}
# Deploy Railway:
...
## B.3 Configurar webhook (produção)
...
3. Em **Webhook URL for incoming messages**, configure a URL completa:
   ```
   # Deploy oficial Docker + Traefik:
   https://${DOMAIN}/webhook/twilio?agent=rhawk_assistant
   # Ou Railway:
...
> **apenas a base publica** (`https://${DOMAIN}` ou `https://api-...up.railway.app`),
> sem path e sem barra final. Misturar esses dois valores quebra a validacao
> de assinatura.
...
curl https://${DOMAIN}/health                           # deploy Docker
# ou
curl https://api-production.up.railway.app/health       # Railway
# {"status":"ok","database":"connected","version":"0.1.0"}
```
...
- [ ] Deploy funcionando (API, Worker, Frontend, DB) — Docker+Traefik via `deploy/` ou Railway
- [ ] `GET /health` retornando 200 em produção
- [ ] Admin panel acessível via `/login` em produção
- [ ] `INTERNAL_SERVICE_TOKEN` configurado com valor forte (32+ chars) em produção
- [ ] `BETTER_AUTH_SECRET` configurado com valor forte (32+ chars) em produção
- [ ] `ADMIN_EMAIL` e `ADMIN_PASSWORD` definidos no Frontend, primeiro login validado e senha trocada em `/settings`

### Execução do cutover

- [ ] Atualizar `TWILIO_FROM_NUMBER` no Worker com o número real
- [ ] Atualizar `TWILIO_WEBHOOK_URL` na API com a URL pública (sem path, sem barra final)
- [ ] Confirmar `OUTBOUND_MODE=real` (ou alias `TWILIO_OUTBOUND_MODE=real`)
- [ ] Habilitar `VALIDATE_TWILIO_SIGNATURE=true` na API
- [ ] Confirmar `TWILIO_AUTH_TOKEN` configurado na API
- [ ] Configurar webhook no Twilio Console → WhatsApp Senders → número real
- [ ] Redeploy: `bash deploy/scripts/deploy.sh` (Docker) ou redeploy Railway
...
> Em `OUTBOUND_MODE=real` (ou alias legacy `TWILIO_OUTBOUND_MODE=real`), o
> Worker faz fail-fast se o canal Twilio estiver "tocado parcialmente" — ou
> seja, alguma das credenciais `TWILIO_ACCOUNT_SID` / `TWILIO_API_KEY_SID` /
> `TWILIO_API_KEY_SECRET` / `TWILIO_FROM_NUMBER` preenchida e outra vazia. Para
> desabilitar o canal Twilio, **zere todas** as credenciais Twilio. Em `mock`,
> o fluxo assíncrono continua funcional, mas o envio outbound é apenas simulado.
...
1. **TWILIO_WEBHOOK_URL incorreta**: deve ser a URL pública exata da API (sem path, sem barra final)
   - Correto: `https://template.vps.illumiai.com` ou `https://api-production.up.railway.app`
   - Errado: `https://template.vps.illumiai.com/` (barra final)
   - Errado: `https://template.vps.illumiai.com/webhook/twilio` (com path)
2. **Webhook salvo no sender com URL divergente**: no Twilio Console, o campo
   **Webhook URL for incoming messages** precisa apontar para a URL completa do
   sender (com path e query):
   - Correto: `https://${DOMAIN}/webhook/twilio?agent=rhawk_assistant`
   - Errado: deixar vazio, usar outro dominio ou esquecer o query param `agent=...`
3. **Redeploy alterou o domínio**: se trocou de DNS ou o Railway gerou novo
   domínio, atualize tanto `TWILIO_WEBHOOK_URL` na API quanto o webhook salvo
   no sender da Twilio
4. **Auth Token desatualizado**: se você regenerou o Auth Token no Twilio Console,
   atualize no `.env.prod` (Docker) ou no Railway
...
3. Verifique logs do worker:
   ...
   ```
4. Confirme que `OUTBOUND_MODE=real` (ou alias `TWILIO_OUTBOUND_MODE=real`) no Worker
...
1. **Twilio Console**: reconfigurar webhook em **Sandbox Settings** (não em WhatsApp Senders)
   ...
   https://random-name.trycloudflare.com/webhook/twilio?agent=rhawk_assistant
   ```
2. **Worker** (`.env.prod` ou Railway): reverter variáveis
   ```bash
   TWILIO_FROM_NUMBER=whatsapp:+14155238886
   OUTBOUND_MODE=mock  # ou real se quiser testar envio pelo sandbox
   ```
3. **API** (`.env.prod` ou Railway): reverter validação
   ```bash
   VALIDATE_TWILIO_SIGNATURE=false
   TWILIO_WEBHOOK_URL=  # limpar ou apontar para cloudflared
   ```
4. Redeploy API e Worker (`bash deploy/scripts/deploy.sh` ou Railway redeploy)
