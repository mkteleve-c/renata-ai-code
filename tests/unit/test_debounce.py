...
class TestMediaFlushPendingText:
    ...
    async def test_flush_sets_process_after_now(self, mock_pool):
        ...
        # Params: phone_number, agent_id, channel (default twilio)
        assert flush_params == ("+5511999999999", "assistant", "twilio")

    async def test_flush_only_same_phone_and_agent(self, mock_pool):
        """Flush é isolado por phone_number + agent_id + channel."""
        pool, conn = mock_pool
        setup_media_with_pending(conn)
        ...
        calls = conn.execute.call_args_list
        flush_params = calls[1][0][1]
        assert flush_params == ("+5511111111111", "bot_a", "twilio")
...
class TestAdvisoryLock:
    ...
    async def test_lock_uses_deterministic_key(self, mock_pool):
        """Chave do lock é determinística para o mesmo phone+agent+channel."""
        import hashlib
        ...
        # Calcula o esperado — inclui o canal (default twilio).
        lock_input = "+5511999999999:assistant:twilio"
        expected = int.from_bytes(
            hashlib.sha256(lock_input.encode()).digest()[:8],
            byteorder="big",
            signed=True,
        )
    ...
    async def test_different_phone_different_lock(self, mock_pool):
        ...
        other_input = "+5511999999999:assistant:twilio"
        other_key = int.from_bytes(
            hashlib.sha256(other_input.encode()).digest()[:8],
            byteorder="big",
            signed=True,
        )
